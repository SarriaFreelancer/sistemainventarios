...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, apiVersion: "v2", count: sales.length, data: sales });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}

// POST: Registrar Venta (v2)
  ...
  try {
    ...
    const newSale = await db.sale.create({
      data: {
        ...
        remarks: remarks || "Venta registrada por API v2",
    ...
    return NextResponse.json({ success: true, apiVersion: "v2", data: newSale }, { status: 201 });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
