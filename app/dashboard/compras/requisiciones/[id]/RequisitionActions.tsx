...
import { updateRequisitionStatus } from "@/app/actions/purchase-requisition-actions";
...
export function RequisitionActions({ requisitionId, status }: RequisitionActionsProps) {
  ...
  const handleStatusChange = async (newStatus: "APPROVED" | "REJECTED") => {
    ...
    const confirmed = await confirmAction(
      `¿${actionName.charAt(0).toUpperCase() + actionName.slice(1)} requisición?`,
      `¿Estás seguro de ${actionName} esta requisición interna?`,
      "Sí, confirmar",
      "Cancelar"
    );
    ...
    setLoading(true);
    const result = await updateRequisitionStatus(requisitionId, newStatus);
  ...
  if (status !== 'PENDING_BOSS' && status !== 'DRAFT') {
    return (
