...
export class SalePage {
  ...
  async clickNewSale() {
    ...
    await this.page.waitForSelector('text=Registrar Nueva Venta');
  }

  async addProductToCart(searchQuery: string, code: string) {
    await this.page.fill('input[placeholder="Escribe código o nombre del producto..."]', searchQuery);
    // Esperar a que se despliegue la lista y hacer clic en el botón del producto
    const productButton = this.page.locator(`button:has-text("${code}")`);
  ...
  async submitSale() {
    await this.page.click('button:has-text("✓ Crear Venta")');
    // Esperar a que se procese y muestre SweetAlert de éxito
    await this.page.waitForSelector('text=¡Venta Registrada!');
